...
class _WinnerScreenState extends State<WinnerScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    //
    // // Sort the playerDataList based on score in descending order
    // List<Map<String, dynamic>> sortedPlayerData = List.from(ludoProvider.playerDataList);
    // sortedPlayerData.sort((a, b) => b['score'].compareTo(a['score']));
    return Scaffold(
      body: Container(
        height: height,
        width: width,
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Stack(
                  ...
                  children: [
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            int.parse(profile.data!.id.toString()).toString() ==
                                int.parse(ludoProvider.firstPlace?.name['id'])
                                    .toString()?
                            "You have won ₹1.7":"You have lost the match",
                            style: TextStyle(
                                color: white,
                          ...
                          Consumer<LudoProvider>(
                            builder: (context, ludoProvider, child) {
                              // List<Map<String, dynamic>> playerScores = ludoProvider.players.map((player) {
                              //   return {
                              //     'type': player.type,
                              ...
                              return ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  print("Mera no${player['name']['number']}");
                                  return Container(
                    ...
                    Padding(
                      padding: EdgeInsets.only(top: int.parse(profile.data!.id.toString()).toString() ==
                          int.parse(ludoProvider.firstPlace?.name['id'])
                              .toString()?height * 0.03:height*0.0),
                      child: Image.asset(
                          int.parse(profile.data!.id.toString()).toString() ==
                                  int.parse(ludoProvider.firstPlace?.name['id'])
                                      .toString()
                              ? Assets.ludoCrown
                              : Assets.imagesLoos,
                      height: int.parse(profile.data!.id.toString()).toString() ==
                          int.parse(ludoProvider.firstPlace?.name['id'])
                              .toString()?null:height*0.25
                      ),
                    ),
                  ],
                )
            ...
            // Text(ludoProvider.determineRankings.toString()),
            Spacer(),
            InkWell(
              onTap: () {
